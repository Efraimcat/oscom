...
*/
  ...
  if (file_exists('includes/functions/version_checker.php')) {
      ...
      $currentVersion = 'Header Tags SEO V 3.3.5';
  ...
  if (isset($_POST['action']) && $_POST['action'] == 'update') {
      if (isset($_POST['x'])) {    
        ...
        while ($pageNumb < count($newfiles) && ! $failedDupSort) {
            ...
            for ($i=0; $i < $languagesCnt; ++$i) {
                ...
                if (! $failedDupSort && isset($_POST[$name])) {
                    $found = true;          
                    $pageTags_query = tep_db_query("select * from headertags where page_name like '" . tep_db_input($newfiles[$pageNumb]['text']) . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
        ...
        for ($x = 2; $x < count($newfiles); ++$x) {    //show the correct boxes
            for ($i=0; $i < $languagesCnt; ++$i) { //show one for each language
                $pageTags_query = tep_db_query("select page_title, page_description, page_keywords, page_logo from headertags where page_name like '" . tep_db_input($newfiles[$x]['text']) . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
                $pageTags = tep_db_fetch_array($pageTags_query); 
      ...
      if ($deleteThisPage) {
          ...
          for ($y = 2; $y < count($newfiles); ++$y) {
             if (strpos($newfiles[$y]['text'], $newfiles[$x]['text']) !== FALSE) { //check for pseudo pages
                ...
                if ($deleted) {
                   for ($i=0; $i < $languagesCnt; ++$i) { //show one for each language
                      $pageDelete_query = tep_db_query("delete from headertags where page_name like '" . $newfiles[$y]['text'] . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
      ...
      if (strpos($psedudoPage, ".php") === FALSE || strpos($psedudoPage, "?") === FALSE || strpos($psedudoPage, "=") === FALSE) {
        ...
        if (in_array($parts[0], $baseFiles)) { //don't allow pseudo pages for base files
           $messageStack->add(sprintf(ERROR_INVALID_PSEUDO_PAGE, $parts[0]), 'error');
            ...
            $pageTags_query = tep_db_query("select * from headertags where page_name like '" . $psedudoPage . "' and language_id = '" . (int)$languages_id . "'");
            $pageTags = tep_db_fetch_array($pageTags_query);
            ...
            if (tep_db_num_rows($pageTags_query) == 0) {
               ...
               AddedToHeaderTagsIncludesFilePseudo($psedudoPage, $fp, $languages_id); //add it if it doesn't exist
...
$(document).ready(function(){
   $.each($("input"),function(){
      ...
      if(typeof id !=='undefined') {
          if (id.includes("pagetitle_") || id.includes("pagedesc_")) {      
       ...
       <tr>
        ...
        <td><table border="0" width="100%">
         ...
         <?php } else if (tep_not_null($versionStatus)) {
           ...
           echo tep_draw_form('version_check', 'header_tags_seo.php', '', 'post') . tep_draw_hidden_field('action', 'getversion');
         ?>
         <tr>
             <td style="float:right"><table border="0" cellpadding="0"><tr>
      ...
      <td align="right"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="BorderedBoxWhite">     
       <tr>
        ...
        <td align="right" width="60%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
         <?php echo tep_draw_form('header_tags', 'header_tags_seo.php', '', 'post') . tep_draw_hidden_field('action', 'update'); ?>
          <tr>
           ...
           <td valign="top" width="100%"><table width="100%" border="1">       
            <tr>
             ...
             <?php if ($currentFile != SELECT_A_FILE && $currentFile != ADD_MISSING_PAGES && count($newfiles) > FIRST_PAGE_ENTRY) { ?>
               <tr> 
                <td align="center"><?php echo (tep_image_submit('button_update.gif', IMAGE_UPDATE) ) . ' <a href="' . tep_href_link('header_tags_seo.php', '') .'">' . '</a>'; ?></td>
               </tr>
             <?php } ?>  
               <tr>
        ...
        <td align="right" width="40%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
         <tr>
          <td align="right" width="40%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
           <?php echo tep_draw_form('header_tags_default', 'header_tags_seo.php', '', 'post') . tep_draw_hidden_field('action', 'default'); ?>
            <tr>
             ...
             <td valign="top" width="100%"><table width="100%" border="2">             
              <tr>
               <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="0">
                 ...
                 <tr> 
                  <td class="smallText" align="center"><INPUT type="image" src="<?php echo 'includes/languages/' . $language . '/images/buttons/button_update.gif'; ?>" NAME="update_default_section"></td>
                 </tr>
                 <tr>
                  <td><?php echo tep_black_line(); ?></td>
                 </tr>          
         ...
         <tr>
          <td align="right" width="40%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
           <?php echo tep_draw_form('header_tags_pseudo', 'header_tags_seo.php', '', 'post') . tep_draw_hidden_field('action', 'default'); ?>
            <tr>
             <td valign="top" width="100%"><table width="100%" border="2">             
              <tr>
               <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="0">
                 ...
                 <tr> 
                  <td class="smallText" align="center"><INPUT type="image" src="<?php echo 'includes/languages/' . $language . '/images/buttons/button_update.gif'; ?>" NAME="add_pseudo_page"></td>
                 </tr>
                 <tr>
                  <td><?php echo tep_black_line(); ?></td>
                 </tr>          
