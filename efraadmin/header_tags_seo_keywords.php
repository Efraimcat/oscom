...
*/
  ...
  include_once('includes/functions/headertags_seo_position_google.php');
...
span.showips { color:#ff0000; cursor:pointer; text-align:center }
</style>
...
$(document).ready(function() {
   ...
   // a show IP item was clicked on 
   $(function () {
     $("span.ip_holder span").on('click', GetIPList); 
     $("span.ip_holder span").click();
   });
});
...
/****************************************
Populate the clicked span item with the 
list of IP's associated with the keyword
****************************************/
function GetIPList() {
  var name = $(this).attr('id');  
  var showing = $(this).text();
  var orig = '<?php echo TEXT_KEYWORD_SHOW; ?>'; 
  if (showing != $(orig).text()) {
     $('#'+name).html('<?php echo TEXT_KEYWORD_SHOW; ?>');;
  } else if (typeof name !== 'undefined') {  
 
     var dataArray = {};
     dataArray[0] = 'getiplist';
     dataArray[1] = name.split("_").pop(); //get the product id
     $.ajax({
        type:  'POST',
        data:  dataArray,
        async: false,
