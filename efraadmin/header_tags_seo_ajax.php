<?php
   ...
   require('includes/languages/' . $language . '/' . 'header_tags_seo_social.php');
   require('includes/languages/' . $language . '/' . 'header_tags_seo_keywords.php');
       ...
       switch ($_POST[0]) {
           ...
           case $_POST[0] == 'sizechange':
              ...
              case $_POST[0] == 'pagination':
                  require_once('includes/functions/headertags_seo_position_google.php');
                  ...
                  $where = $_POST[4];
                  $keywords_query = tep_db_query("select * from headertags_keywords " . $where . " order by " . $orderBy . " LIMIT $position, " . MAX_DISPLAY_SEARCH_RESULTS);
                  ...
                  if (tep_db_num_rows($keywords_query) > 0 ) {
                      ...
                      while ($keywords = tep_db_fetch_array($keywords_query)) {
                          $assignedID = (tep_not_null($searchWords) ? GetProductID($searchWords, $keywords['keyword'], $keywords['language_id']) : '');
                          ...
                          $arry['div'] .=   '<tr>
                                   ...
                                   <td>' . '<span style="color:' . $keyword_color . '; font-weight:bold;">' . $keyword . '</span><br /><span class="ip_holder"><span id="show_ips_' . $keywords['id'] . '">' . TEXT_KEYWORD_HIDE . '</span></span></td>
              ...
              /**** BEGIN GET IP LIST ****/
              case $_POST[0] == 'getiplist': 
                  $ipList = GetKeywordIPs($_POST[1]);
