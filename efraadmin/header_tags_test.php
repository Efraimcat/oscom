...
*/
  ...
  require('includes/languages/' . $language . '/header_tags_seo.php');
 
  $dbError = '';
  $filename = DIR_FS_CATALOG. 'includes/header_tags.php'; 
  ...
  if (isset($_POST['action']) && $_POST['action'] == 'test')  {
    ...
    if (GetPermissions(DIR_FS_CATALOG_IMAGES) != Getpermissions($filename)) {
      ...
      $results[] = sprintf(ERROR_WRONG_PERMISSIONS, $filename, Getpermissions('images/'));
    ...
    $files[] = DIR_FS_ADMIN . 'header_tags_seo_ajax.php';
    $files[] = DIR_FS_ADMIN . 'header_tags_seo_cron.php';
    $files[] = DIR_FS_ADMIN . 'header_tags_seo_help.php';
    $files[] = DIR_FS_ADMIN . 'header_tags_seo_keywords.php';
    $files[] = DIR_FS_ADMIN . 'header_tags_seo_silo.php';
    $files[] = DIR_FS_ADMIN . 'header_tags_seo_social.php';
    $files[] = DIR_FS_ADMIN . 'headertags_seo_exclude.php';
    $files[] = DIR_FS_ADMIN . 'headertags_seo_keyword_position.php';
    $files[] = DIR_FS_ADMIN . 'includes/header_tags_seo_words.txt';  
    $files[] = DIR_FS_ADMIN . 'includes/functions/header_tags.php';          
    $files[] = DIR_FS_ADMIN . 'includes/boxes/header_tags_seo.php';
    $files[] = DIR_FS_ADMIN . 'includes/languages/' . $language . '/header_tags_seo.php';
    $files[] = DIR_FS_CATALOG . 'includes/header_tags.php';    
    $files[] = DIR_FS_CATALOG . 'includes/functions/clean_html_comments.php';
    $files[] = DIR_FS_CATALOG . 'includes/functions/header_tags.php';
    $files[] = DIR_FS_CATALOG . 'includes/modules/boxes/bm_header_tags.php';
    $files[] = DIR_FS_CATALOG . 'includes/modules/header_tags_social_bookmarks.php';
    ...
    $check_query = tep_db_query("SHOW TABLES LIKE 'headertags'");
    ...
    $dbError = false; //for master group of database errors
    $check_query = tep_db_query("select page_name, page_title, language_id from headertags where page_title like 'New Home Page Title' or page_title like 'products new home'");
    ...
    for ($i = 0; $i < count($languages); ++$i)
    {
      ...
      while ($check = tep_db_fetch_array($check_query))
      {
        if (! $dbError)
        {
          ...
          $results[] = '<a class="htc_Link" href="' . tep_href_link('categories.php', 'action=new_product&pID=' . $check['products_id']) . '">' . sprintf(ERROR_DUPLICATE_TITLE_LANGUAGE, $languages[$i]['name']) . '</a>';
    ...
    if (tep_not_null(($msg = CheckForMissingTags(true, true)))) {
      ...
      for ($i = 0; $i < count($languages); ++$i) {    
        ...
        while ($check = tep_db_fetch_array($check_query)) {
          ...
          if (! $dbError) {
            ...
            $results[] = sprintf($msg, '<a class="htc_Link" href="' . tep_href_link('categories.php', 'action=new_product&pID=' . $check['products_id']) . '">' . (tep_not_null($check['products_head_title_tag']) ? $check['products_head_title_tag'] : ERROR_NO_NAME) . '</a>',   strlen($check['products_head_title_tag']));
            $dbError = true;       
          }
          else
          {
      ...
      {      
        ...
        while ($check = tep_db_fetch_array($check_query))
        {
          ...
          if (! $dbError)
          {
            ...
            $results[] = sprintf(ERROR_DUPLICATE_TITLE_LANGUAGE, $languages[$i]['name']);          
            $results[] = sprintf($msg, '<a class="htc_Link" href="' . tep_href_link('categories.php', 'action=new_product&pID=' . $check['products_id']) . '">' . (tep_not_null($check['products_head_title_tag']) ? $check['products_head_title_tag'] : ERROR_NO_NAME) . '</a>',   strlen($check['products_head_desc_tag']));
            $dbError = true;       
          }
          else
          {
    ...
    for ($i = 0; $i < count($pseudoArray); ++$i)
    {
        ...
        {
            $db_query = tep_db_query("select 1 from headertags where page_name LIKE '" . $pseudoArray[$i] . '?' . "%' limit 1");
    ...
    $check_query = tep_db_query("select page_name from headertags where language_id = '" . (int)$languages_id . "'");
...
//    else if ($notFirst && $i == 0)  //show what is there
  ...
  else if (isset($_POST['action']) && $_POST['action'] == 'delete_extra_files')
  {
     ...
     for ($i=0; $i < count($files); ++$i) 
       tep_db_query("delete from headertags where page_name LIKE '" . $files[$i] . "'");
  }  
...
<table border="0" width="100%" cellspacing="2" cellpadding="2">
  <tr>
    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <td align="right"><table width="100%" border="0" cellspacing="0" cellpadding="0">     
       ...
       <tr>
        <td align="right" width="60%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
         <?php echo tep_draw_form('header_tags', 'header_tags_test.php', '', 'post') . tep_draw_hidden_field('action', 'test'); ?></td>
          ...
          <tr>
           <td><table border="0" style="border:ridge gray 3px;" width="100%">
            ...
            <tr> 
             <td align="center"><?php echo (tep_image_submit('button_test.gif', 'Test') ) . ' <a href="' . tep_href_link('header_tags_seo.php', '') .'">' . '</a>'; ?></td>
            </tr>
           </table></td>
          </tr> 
     ...
     <tr><td><?php echo tep_image('images/pixel_trans.gif', '', '100%', '4'); ?></td></tr>
     <tr><td><?php echo tep_image('images/pixel_black.gif', '', '100%', '4'); ?></td></tr>
     <tr>
