...
function AddMissingPages($languages_id = 1, $languages) {
    ...
    $pageTags_query = tep_db_query("select page_name from headertags where language_id = '" . (int)$languages_id . "'");
    ...
    if ($fp) {
        while(false !== ($file = readdir($dir))) {
            if(! is_dir($file) && substr($file, -4, 4) === ".php") { //only look at php files                  
                if (($isTemplate || ($result = FileNotUsingHeaderTags($file) === 'FALSE'))) { //header tags title code is in file
        ...
        for ($i = 0; $i < $cntNewFiles; ++$i) {
            if (! in_array($newfiles[$i], $existingInDB)) {
                for ($t = 0; $t < count($languages); ++$t) {
                    ...
                    tep_db_perform('headertags', $sql_data_array);
                }
            }
        }  
    }
    ...
    $pseudoFiles = array();
    $db_query = tep_db_query("select page_name from headertags where page_name LIKE '%?%'");
...
function AddedToHeaderTagsIncludesFile($file, &$fp, $languages_id, $notDuplicatePage) { //add a new entry to the includes/header_tags.php file if not present
   ...
   if ($notDuplicatePage) {
      ...
      $incArray['line'] .= sprintf("      \$header_tags_array = tep_header_tag_page('%s');\n",$file );
...
function AddedToHeaderTagsIncludesFilePseudo($file, &$fp, $languages_id) { //add a new entry to the includes/header_tags.php file if not present
    ...
    for ($idx = 0; $idx < $cnt; ++$idx) { //find where to insert the new page
        ...
        if (! empty($section)) {
            ...
            $cmp = strcasecmp($section, $parts[0]); 
            if ($cmp > 0) {
                if ($insertPoint == 0) {
                    $insertPoint = $idx; 
                } 
            } else if ($cmp == 0) {  //don't care about < 0
                $insertPoint = $idx; 
                break;
            }           
        } else if (strpos($fp[$idx], "// ALL OTHER PAGES NOT DEFINED ABOVE") !== FALSE) { 
    ...
    if (! NotDuplicatePage($fp, $parts[0])) { // an entry exists for this page - delete if it is for the main page
        ...
        foreach ($fp as $l => $d) {
            if (! $fline && strpos($d, '// ' . $parts[0]) !== FALSE) {
                $fline = $l;  //beginning of the section for this page
            }
            if ($fline && strpos($fp[$l], 'includes/modules/header_tags_pseudo.php') !== FALSE) {         
                return false; //this is the correct method for this page so exist
            }               
            if ($fline && strpos($fp[$l], 'ReadCacheHeaderTags') !== FALSE) {
                $mline = $l;  //middle of the section for this page     
            } 
            if ($mline && strpos($fp[$l], 'break;') !== FALSE) {
                $lline = $l;  //last of the section for this page
                break;
            } 
        }
        $addPseudoCode = true;
    ...
    if ($fline && $mline && $lline) { //this is the old style of code so remove it
        unset($fp[$lline], $fp[$mline], $fp[$fline]);
    } else if ($fline) {              //an entry exists and its's the new style so just return
        return false;
    }        
    ...
    if ($addPseudoCode) {  //pseudo code already exists so don't add again    
        ...
        for ($d = $endLine; $d > $insertPoint + 1; --$d) {
            unset($fp[$d]);
        }          
    }

    $incArray = array();
    $incArray['pseudo'] = sprintf("\n// %s\n", $parts[0]); 
    $incArray['pseudo'] .= sprintf("  case (basename(\$_SERVER['SCRIPT_FILENAME']) === '%s'):\n", $parts[0]);        
    $incArray['pseudo'] .= "    include('includes/modules/header_tags_pseudo.php');" . "\n";
...
function CheckForMissingTags($fullList = false, $checkDesc = false, &$justAmt = array()) {
  /***************** Check Products ***************/
  $missingStr = '';
  $products_query = tep_db_query("select p.products_id, pd.products_name, pd.language_id from products p left join products_description pd on (p.products_id = pd.products_id) where p.products_status = 1 and (pd.products_head_title_tag = '' or pd.products_head_title_tag IS NULL or pd.products_head_desc_tag = '' or pd.products_head_desc_tag IS NULL or pd.products_head_keywords_tag = '' or pd.products_head_keywords_tag IS NULL )");
  if (tep_db_num_rows($products_query) > 0)   {
    if ($fullList) {
      ...
      while ($products = tep_db_fetch_array($products_query)) {
        ...
        $missingStr .= '<td><a class="htc_Link" href="' . tep_href_link('categories.php', 'action=new_product&pID=' . $products['products_id']) . '">' . (tep_not_null($products['products_name']) ? $products['products_name'] : ERROR_NO_NAME) . '</a></td>';
        $missingStr .= '</tr>';
      }
    } else {
  ...
  $categories_query = tep_db_query("select categories_id, categories_name, language_id from categories_description where (categories_htc_title_tag = '' or categories_htc_title_tag IS NULL or categories_htc_desc_tag = '' or categories_htc_desc_tag IS NULL or categories_htc_keywords_tag = '' or categories_htc_keywords_tag IS NULL $desc)");
  if (tep_db_num_rows($categories_query) > 0) {
    if ($fullList) {
      ...
      while ($categories = tep_db_fetch_array($categories_query)) {
        $missingStr .= '<tr class=smallText">';
        $missingStr .= '<td><a class="htc_Link" href="' . tep_href_link('categories.php', 'cPath=&cID=' . $categories['categories_id']) . '">' . (tep_not_null($categories['categories_name']) ? $categories['categories_name'] : ERROR_NO_NAME) . '</a></td>';
        $missingStr .= '</tr>';      
      }
    } else { 
  ...
  if (tep_db_num_rows($manufacturers_query) > 0) {
    if ($fullList) {
      ...
      while ($manufacturers = tep_db_fetch_array($manufacturers_query)) {
        $missingStr .= '<tr class=smallText">';
        $missingStr .= '<td><a class="htc_Link" href="' . tep_href_link('manufacturers.php', 'page=1&mID=' . $manufacturers['manufacturers_id']) . '">' . (tep_not_null($manufacturers['manufacturers_name']) ? $manufacturers['manufacturers_name'] : ERROR_NO_NAME) . '</a></td>';
        $missingStr .= '</tr>';       
      }
    } else {
...
function FileNotUsingHeaderTags($file) {
  ...
  if (($fp = @file($file))) {
    for ($i = 0; $i < count($fp); ++$i) {
      if (strpos($fp[$i], 'header_tags.php') !== FALSE ) {
        return 'FALSE';
      }  
    }
    return 'TRUE';
...
function GetFileList($languages_id = 1) {
    ...
    if ($handle = @opendir(DIR_FS_CATALOG)) {
         ...
         while (false !== ($file = readdir($handle))) { 
             ...
             if (($isTemplate || ($result = FileNotUsingHeaderTags($file) === 'FALSE'))) {
                 $pageTags_query = tep_db_query("select * from headertags where page_name like '" . tep_db_input($file) . "' and language_id = '" . (int)$languages_id . "'");
         ...
         $pageTags_query = tep_db_query("select * from headertags where page_name like '%?%' and language_id = '" . (int)$languages_id . "'");
         while ($pageTags = tep_db_fetch_array($pageTags_query)) {
...
/********************************************************************************
Returns a list of IP's used in keyword searches
********************************************************************************/
function GetKeywordIPs($id) {
    $ipStr = ''; 
    $db_query = tep_db_query("select INET_NTOA(ip_number) as ip from headertags_ip_tracker where id_reference = '" . (int)$id . "'");
    if (tep_db_num_rows($db_query)) {
        while ($db = tep_db_fetch_array($db_query)) {
            $ipStr .= $db['ip'] . "<br>";
        }        
    }
    $ipStr .= TEXT_KEYWORD_HIDE;
    return $ipStr;
} 
/********************************************************************************
...
function GetMetaInfo($pageName) {
    ...
    if (substr_count($pageName, '?') > 1) { // this is a pseudo page
        ...
        $lang_query = tep_db_query("select languages_id from languages where code = '" . tep_db_input($lang) . "'");
        $lang = tep_db_fetch_array($lang_query);
        $name = $parts[0] . '?' . $parts[1];
        $tags_query = tep_db_query("select page_title as title, page_description as description, page_keywords as keywords from headertags where page_name = '" . tep_db_input($name) . "' and language_id = " . (int)$lang['languages_id']);
...
function GetSearchCounts() {
    ...
    $db_query = tep_db_query("select count(*) as ttl_words, SUM(found=1) as found_words, SUM(found=0) as notfound_words from headertags_keywords where language_id = '" . (int)$languages_id . "'");
...
function GetSocialIconsArray() {
  $allicons = glob(DIR_FS_CATALOG . 'images/socialbookmark/*.png');
  $icons = array();
...
function IsOptionSet($option) {
    $db_query = tep_db_query("select 1 from headertags_default where " . $option . " = 1");
    return tep_db_num_rows($db_query);
}
 
function IsTemplate() { //return if BTS is installed or a 2.3 shop
    if (HEADER_TAGS_BYPASS_ISTEMPLATE == 'true' || defined('DIR_WS_TEMPLATES') || defined('DIR_WS_TEMPLATES_DEFAULT'))   //BTS is installed
        return true;

    if (file_exists('includes/template_top.php')) //this is a 2.3 shop
        return true;                    
...
function ResetCache_HeaderTags($name, $id, $deleteAll = false) {
    ...
    $cache_query = tep_db_query("select title from headertags_cache");
        ...
        for ($j=0, $k=sizeof($languages); $j<$k; $j++) {
            ...
            if ($deleteAll) {
                ...
                if (strpos($cacheExt, $cacheFileName) !== FALSE && (tep_not_null($id) ? strpos($cacheExt, $id) !== FALSE : true)) {
                    tep_db_query("delete from headertags_cache where title = '" . $cache['title'] . "'");
                }
            } else {
...
function SetBaseFilesToDefaultLanguage($languages_id = 1) { //switch the headertags language settings in the original
  ...
  for ($i = 0; $i < count($baseFiles); ++$i) {
    $pageTags_query = tep_db_query("select page_name from headertags where page_name like '" . $baseFiles[$i] . "' and language_id = '" . (int)$languages_id . "' LIMIT 1");

    if (tep_db_num_rows($pageTags_query) == 0) { 
      $pageTags_query = tep_db_query("select page_name from headertags where page_name like '" . $baseFiles[$i] . "' and language_id = '1' LIMIT 1");
      if (tep_db_num_rows($pageTags_query) > 0) {
        tep_db_query("update headertags set language_id = '" . $languages_id . "' where page_name like '" . $baseFiles[$i] . "'"); 
      }
    }
  }
}
