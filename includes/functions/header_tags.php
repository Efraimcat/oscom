...
function tep_header_tag_page($file) {
  ...
  $pageTags_query = tep_db_query("select * from headertags where page_name like '" . $file . "' and language_id = '" . (int)$languages_id . "'");
...
function GetCanonicalURL() {
  ...
  if ($cnt == 1 && basename($_SERVER['SCRIPT_FILENAME']) === 'index.php') { //home page
     ...
     if (tep_not_null($cPath) || (isset($_GET['manufacturers_id']) && $category_depth == 'top')) {
        ...
        return StripSID(tep_href_link('index.php', $args, $request_type, false) );
     }
...
function GetHTSTagCloudLink($keyword, $languages_id) {
    ...
    $kwrd = tep_db_prepare_input($keyword);
    $kwrd = tep_db_input($kwrd);

    $searchDesc = '';
    if (file_exists('includes/template_top.php')) {  //this is a 2.3 shop
        $boxLocn = 'includes/modules/boxes/bm_search';
    } else {
    ...
    $hts_tags_query = tep_db_query("select product_id from headertags_search where keyword = '" . $kwrd . "' and language_id = '" . (int)$languages_id . "'");
    if (tep_db_num_rows($hts_tags_query) > 0) {
        $id = tep_db_fetch_array($hts_tags_query);
        return tep_href_link('product_info.php', 'products_id=' .$id['product_id']);          
    }     

    $hts_tags_query = tep_db_query("select p.products_id from products p left join products_description pd on p.products_id = pd.products_id where ( pd.products_head_keywords_tag like '%" . $kwrd . "%' or pd.products_name like '%" . $kwrd . "%' or p.products_model like '%" . $kwrd . "%'" . $searchDesc . " ) and p.products_status = 1 " . $quotes . " and pd.language_id = '" . (int)$languages_id . "' limit 1");
    if (tep_db_num_rows($hts_tags_query) > 0) {
        $id = tep_db_fetch_array($hts_tags_query);
        return tep_href_link('product_info.php', 'products_id=' .$id['products_id']);          
    }     

    $hts_tags_query = tep_db_query("select categories_id from categories_description where ( categories_htc_keywords_tag like '%" . $kwrd . "%' or categories_name like '%" . $kwrd . "%' ) and language_id = '" . (int)$languages_id . "' limit 1");
    if (tep_db_num_rows($hts_tags_query) > 0) {
        $id = tep_db_fetch_array($hts_tags_query);
        return tep_href_link('index.php', 'cPath=' .$id['categories_id']);          
    }     

    $hts_tags_query = tep_db_query("select m.manufacturers_id from manufacturers m inner join manufacturers_info mi on m.manufacturers_id = mi.manufacturers_id where ( mi.manufacturers_htc_keywords_tag like '%" . $kwrd . "%' or m.manufacturers_name like '%" . $kwrd . "%' ) and mi.languages_id = '" . (int)$languages_id . "' limit 1");   
    if (tep_db_num_rows($hts_tags_query) > 0) {
        $id = tep_db_fetch_array($hts_tags_query);
        return tep_href_link('index.php', 'manufacturers_id=' .$id['manufacturers_id']);          
    }     
...
function ReadCacheFromDB(&$data, $name, $gzip = 0) {
   ...
   $cache_query = tep_db_query("select * from headertags_cache where title = '" . $name . "' limit 1");
...
function WriteCacheToDB($data, $name) {
   ...
   $cache_query = tep_db_query("select 1 from headertags_cache where title = '" . $name . "' limit 1");

   if (tep_db_num_rows($cache_query) == 0)
       tep_db_query("insert into headertags_cache ( title, data ) values ('" . $name . "', '" . $data . "')");
   else
       tep_db_query("update headertags_cache set data = '" . $data . "'");
}
