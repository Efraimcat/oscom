...
//	EBG 06/06/17
//
//	para B2B al iniciar sessión recogemos de la BD el NIF. Despues en   function tep_get_tax_rate (general.php) hacemos la comprobación y aplicamos
//	0% de IVA si tiene CIF y su pais de origen no es España
//
//    $customer_info_query = tep_db_query("select c.customers_firstname, c.customers_default_address_id, ab.entry_country_id, ab.entry_zone_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " ab on (c.customers_id = ab.customers_id and c.customers_default_address_id = ab.address_book_id) where c.customers_id = '" . (int)$login_customer_id . "'");
	$customer_info_query = tep_db_query("select c.customers_firstname, c.customers_default_address_id, c.entry_nif, ab.entry_company, ab.entry_country_id, ab.entry_zone_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " ab on (c.customers_id = ab.customers_id and c.customers_default_address_id = ab.address_book_id) where c.customers_id = '" . (int)$login_customer_id . "'");
...
// EBG 06/06/17	registramos el NIF  y el nombre de la empresa del cliente al inicio de sessión en GLOBAL
	$customer_nif = $customer_info['entry_nif'];
	tep_session_register('customer_nif');
	$customer_company = $customer_info['entry_company'];
